...
const columns = [
  ...
  { field: "identifier", headerName: "Identifier", width: 233 },
  ...
  useEffect(() => {
    if (hotelReducer.systemLogs) {
      const systemLogs = hotelReducer.systemLogs.filter(
        (systemLog) => systemLog.type === "SYSTEM_LOG"
      );
              ...
              .map((row, index) => {
                return (
                  ...
                  >
                    ...
                    <TableCell>
                      {" "}
                      {row.action ? row.module + " : " + row.action : "N/A"}
                    </TableCell>
                    <TableCell>
                      <TableRowStatus data={row.contentChange} />
                    </TableCell>
