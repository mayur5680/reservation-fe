...
export const DiningOptions = (props) => {
  ...
  useEffect(() => {
    if (props.dinningOptions) {
      convertDiningData(props.dinningOptions);
    }
  }, [props.dinningOptions]);
  ...
  const convertDiningData = (preorders) => {
    const data = preorders.map((preorder) => {
      return createData(
        ...
        preorder.price,
        preorder.diningOptionQty,
        preorder.total,
        preorder.originalTotalAmount,
        preorder.image
      );
    });
    setDiningOption(data);
    return data;
  };

  useEffect(() => {
    if (diningOption.length > 0) {
      if (page * rowsPerPage >= diningOption.length) {
        setPage(page - 1);
      }
    }
  }, [diningOption]);
  ...
  return (
    <div className="w-100 p-10">
      <Typography sx={{ fontWeight: "bold" }}>Dining Options</Typography>
      <Paper sx={{ width: "100%" }}>
        <TableContainer sx={{ maxHeight: "calc(100vh - 377px)" }}>
          ...
          >
            ...
            <TableBody>
              {stableSort(diningOption, getComparator(order, orderBy))
